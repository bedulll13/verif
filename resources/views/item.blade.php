                ...
                <div class="relative w-full overflow-auto">
                    <div class="flex justify-between items-center">
                        ...
                        <a href="/{{ $code }}/dashboards/item/new">
                            <button class="bg-slate-400 rounded-lg text-white px-4 py-2 hover:bg-red-500">+ Tambah Data</button>
                        </a>
                    </div>
                  <table class="w-full caption-bottom text-sm">
                    <thead class="[&amp;_tr]:border-b">
                      <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        ...
                        <th class="h-12 px-4 align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0">
                          Action
                        </th>
                      </tr>
                    </thead>
                    <tbody class="[&amp;_tr:last-child]:border-0">
                      ...
                      <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        ...
                        <td class="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">{{$item->part_name}}</td>
                        @if($customer->id==1)
                        <td class="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">{{$item->job_no}}</td>
                        @endif
                        <td class="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">{{$item->qty}}</td>
                        <td class="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
                          <div class="flex flex-col gap-3">
                            <a href="/{{ $code }}/dashboards/item/{{$item->part_id}}" class="w-full">
                              <button class="px-6 py-1 rounded bg-zinc-400 text-white"> edit</button>
                            </a>
                            <form action="/{{$code}}/dashboards/item/{{$item->part_id}}/delete" method="post">
                              @csrf
